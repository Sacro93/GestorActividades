...
@Composable
fun ListadoTareas(
    viewModel: ListadoTareasViewModel, onBack: () -> Unit // Función para volver al menú principal
) {
    ...
    Column(
        ...
        if (tareas.isEmpty()) {
            ...
            LazyColumn(
                ...
                items(tareas) { tarea ->
                    ItemTarea(tarea = tarea, onMarcarRealizada = { id ->
                        viewModel.marcarTareaComoRealizada(id = id, onSuccess = {
                            Toast.makeText(
                                context, "Tarea marcada como realizada", Toast.LENGTH_SHORT
                            ).show()
                        }, onFailure = { exception ->
                            Toast.makeText(
                                context,
